...
$osvhdsize = Get-Attr -obj $params -name osvhdsize_bytes -default 50000000000
$osdiskpath = Get-Attr -obj $params -name osdiskpath -default $null
...
Function VM-Create {
    ...
    if (!$CheckVM) {
        $cmd = "New-VM -Name $name -Path $vmpath"
        ...
        if ($osdiskpath) {
            ...
            if (Test-Path $osdiskpath) {
                $cmd += " -VHDPath $osdiskpath"
            } else {
                $cmd += " -NewVHDPath $osdiskpath –NewVHDSizeBytes $osvhdsize"
            }
        }
